# Runbook — sdkwork-manager 部署 / 升级 / 回滚（中文）

适用环境：`development|test|staging|demo|production`。所有命令默认在本机 WSL 执行，远程主机加 `--host ssh://[user@]host[:port]`。镜像参考：`registry.sdkwork.com/apps/sdkwork-manager-standalone:0.1.0`（tag 取自 `sdkwork.app.config.json` → `release.currentVersion`）。

> ℹ️ **交付姿态**：本模块 standalone 交付为宿主包（`host-package/host-service`），cloud 交付为容器镜像（`configuration-bundle/nginx`）。
...
install 会同步 bundle 到 `/opt/deploy/sdkwork-manager/bundle`，加载镜像，按实例启动并等待健康门禁（`/healthz`）。
