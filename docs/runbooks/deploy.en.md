# Runbook — sdkwork-manager deploy / upgrade / rollback (EN)
...
targets. Image reference: `registry.sdkwork.com/apps/sdkwork-manager-standalone:0.1.0`
(tag from `sdkwork.app.config.json` → `release.currentVersion`).

> ℹ️ **Delivery posture**: standalone delivery is a host package (`host-package/host-service`); cloud delivery is a container image (`configuration-bundle/nginx`).
...
install syncs the bundle to `/opt/deploy/sdkwork-manager/bundle`, loads the image, starts
